...
- We have $n$ points in a plane region.
- Each point is classified as *Group A* or *Group B*.
- **Problem:** Quantify (or describe) the extent to which the two groups are "segregated".

. . .

In practice: data consists of counts for small subregions (e.g. census block groups).

## Sociologists love numerical indexes
...
## {data-background="images/checsurftop.png" data-background-size="contain"}

## {data-background="images/checksurf.png" data-background-size="contain"}

## Kernel Density Estimation

- Idea: Given $n$ points, piece together a surface by combining $n$ *bivariate normal distributions.*
    - Up to some adjustable constants, these surfaces basically have the form

$$
f(x,y) = e^{-(x^2 + y^2)}
$$

## {data-background="images/bivnorm.png" data-background-size="contain"}

## {data-background="images/kde2dwiki.png" data-background-size="contain"}
...
>- From this data we compute kernel density estimates for:
